...
// Api Controller for this application
class Api_Controller extends REST_Controller {

    // public $ORG =array();
    public $ORGID = '';
    public $ORG = array();
    public $RESELLER =array();
    public $USER =array();
    public $RESPONSE=array();
    public $RESPONSE_CODE='code';
    public $RESPONSE_TEXT='message';
    public $RESPONSE_SMSID='smsid';
    public $REQUEST_TEXT='request';
    ///------------------------------------------------
    public $TEXT_700='Message Sent';
    public $TEXT_701='Operational Successfull';
    public $TEXT_600='Invalid API Key Or Mobile';
    public $TEXT_601='Account Suspended / Blocked';
    public $TEXT_602='Invalid Masking';
    public $TEXT_603='Suspended Mask Label';
    public $TEXT_604='Package Expired';
    public $TEXT_605='Unsupported Api Operation With Current Account Status';
    public $TEXT_606='Insufficient SMS Credits';
    public $TEXT_607='Insufficient Account Balance';
    public $TEXT_608='Invalid Mobile Number';
    public $TEXT_609='Missing Required Parameters';
    public $TEXT_610='System Error! Please Try Again Later';
    public $TEXT_611='Invalid Message Type! Select branded or nonbranded';
    public $TEXT_612='Past Date! Provide date in future time';
    public $TEXT_613='Invalid License Key';
    public $TEXT_614='Invalid smsid';
    public $TEXT_615='Receiver Maximum Limit Reached!';
    public $TEXT_YES='yes';
    public $TEXT_NO='no';
    //-------------------------------------------------
    public $TYPE_BRANDED='brand';
    public $TYPE_NONBRANDED='nonbrand';
    public $OUTPUT_TEXT='text';
    public $OUTPUT_JSON='json';
    public $REQ_BACK_SHOW='show';
    public $REQ_BACK_HIDE='hide';

     function __construct(){
        // Construct the parent class
        parent::__construct();
        $models = array('organization_m','user_m','sms_brand_queue_m','user_package_m','masking_m','org_customer_m','org_masking_m','org_customer_package_m');
        $this->load->model($models); 
        
        $this->tools->set_base_url();
        $organization = $this->tools->get_org_id();
        if (!$organization) {
            //authentication problem. Set the response and exit
            $this->send_response();
        } else {
            $this->ORGID = $organization; //org id
            $this->ORG = $this->organization_m->get_row_by_id($this->ORGID);
            $this->RESELLER=$this->user_m->get_row_by_id($this->ORG->user_id);
        }

        

        //perform basic authentication                   
        $form = $this->input->safe_post_get(array('mobile', 'apikey'));
        $mobile=get_standard_mobile_number($form['mobile']);
        $filter=array('mobile'=>$mobile,'apikey'=>$form['apikey'],'user_id'=>$this->RESELLER->id);
         if($this->org_customer_m->get_num_rows($filter)>0){
            $this->USER=$this->org_customer_m->get_row_by_fields($filter);
         }else{
            //authentication problem. Set the response and exit
            $this->RESPONSE[$this->RESPONSE_CODE]=600;$this->RESPONSE[$this->RESPONSE_TEXT]=$this->TEXT_600;
            $this->send_response();
    ...
    protected function send_response(){
        $form = $this->input->safe_post_get(array('output'));
        if($form['output']=='json'){
                $this->response($this->RESPONSE, REST_Controller::HTTP_OK);
        }else{
                echo $this->RESPONSE[$this->RESPONSE_TEXT];
        }
        exit(1);
    }
