...
class Fabsam{
	...
	function __construct(){
        ...
        $this->_host_url="https://license.fabsam.com";
    ...
    public function get_metadata($app_code=''){
        ...
        $url=$this->_host_apiurl."getdata/products?apikey=".urlencode($this->_host_apikey)."&code=".urlencode($code);
    ...
    public function get_licenseinfo($licensekey=''){
        ...
        $url=$this->_host_apiurl."getdata/licenses?apikey=".urlencode($this->_host_apikey)."&licensekey=".urlencode($licensekey);
        $response=$this->explore_url($url);
        return $response;
    }
    ...
    public function is_valid_envatocode($code){
        $url=$this->_host_apiurl."validate/envatocode?apikey=".urlencode($this->_host_apikey)."&envatocode=".urlencode($code);
        $response=$this->explore_url($url);
    ...
    function download_updates($filename,$destination){
        if($this->is_connected()){
            ...
            $file_url=$this->_host_url.'/assets/downloads/'.$metadata->data[0]->code.'-'.$metadata->data[0]->version_code.'.zip';
