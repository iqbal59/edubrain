...
<div class="wrapper _bg4586">
	...
	<div class="sa4d25">
		<div class="container-fluid">			
			<div class="row">
				<div class="col-lg-12">	
					<h2 class="st_title"><i class="uil uil-book-alt"></i>MCQ Test Results</h2>
				...
				<div class="table-responsive mt-30">
					<table class="table ucp-table">
						...
						<tbody>
							...
							$filter['jd <=']=$todayjd;
			                $std_answers=$this->quiz_answer_m->get_values_array('question_id','answer',array('student_id'=>$this->LOGIN_USER->mid));
							$papers=$this->quiz_m->get_rows($filter,array('select'=>'name,date,jd,subject_id,class_id,section_id,start_time,marks','orderby'=>'jd DESC, start_time DESC'));
							foreach($papers as $ppr){
								...
								if($this->subject_group_m->get_rows($filter,'',true)<1){
									...
									$ppr_questions=$this->quiz_question_m->get_rows(array('quiz_id'=>$ppr['mid']),array('select'=>'quiz_id,marks,answer'));
									$i++; 
									?>
									<tr>
									...
									<?php if($this->quiz_attempt_m->get_rows(array('quiz_id'=>$ppr['mid'],'student_id'=>$this->LOGIN_USER->mid),'',true)<1){$total_skipped++;
									 ?>
										<td class="text-center" colspan="3"><span class="text-danger">Test Not Attempted</span></td>
									<?php }else{
										$obt_marks=0;
										$total_marks=0;
										$percentage=0;
										...
										foreach($ppr_questions as $q){
											//verify the answer
											if(isset($std_answers[$q['mid']]) && $std_answers[$q['mid']]==$q['answer']){
												//correct answer
												$obt_marks+=$q['marks'];
												$grand_obt_marks+=$q['marks'];
											}
										}
                    ...
                    <?php if($i>0){ ?>
					<p>The student not appeared in <strong><span class="text-danger"><?php print $total_skipped; ?></span></strong> tests out of total <strong><?php print $i; ?></strong> tests.</p>
					<?php } ?>
				</div>
			</div>	
