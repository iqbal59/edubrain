...
<!-- Header Start -->
		...
		<div class="container exam_container">
			<div class="row soft_padding">
				<div class="col-lg-10 col-md-10">
					<a href="<?php print $quiz_url.'/?finish';?>" onclick="return confirm('Are you sure to end paper? You can not resume paper afterwards.');" class="btn btn-danger">End Quiz</a>
					<?php if($quiz->qbase_time>0){ ?>	
			...
			<div class="container exam_container">
				<div class="row soft_padding">				
						...
						<div class="right_side">
							<div class="fcrse_2 mb-30">
								<div class="tutor_content_dt">
									<div class="tutor150">
										<a class="tutor_name"><span class="color-white">Paper Information</span></a>
									</div>
									<hr>
									<div class="tutor_cate">
										<strong>Quiz Name : </strong><?php print ucwords($quiz->name);?> <br>
									...
									<hr>
										...
										<a href="<?php print $quiz_url.'/?prev';?>" class="btn btn-info"><i class="uil uil-angle-double-left"></i>Previous</a> 
										<?php endif ?>
										| 
										<?php if ($current_question+1<count($questions)): ?>
										<a href="<?php print $quiz_url.'/?next';?>" class="btn btn-info">Next <i class="uil uil-angle-double-right"></i></a>
										<?php endif ?>
									<?php } ?>
								</div> 
							</div>
					...
					<div class="col-lg-9 col-md-9 right_bar">
						<form action="<?php print $this->CONT_ROOT.'attempt/'.$quiz->mid;?>" method="post">
							...
							<div class="certi_form">
								...
								<span class="float-right">
									<button class="test_submit_btn" type="submit">Save Answer</button>
								</span>
								<?php } ?>
								<span class="float-right mr-10">
									<button class="test_submit_btn" type="submit" name="next" value="yes">Save &amp; Next</button>
								</span>
							</div>
						</form>
					</div>	
					...
					<div class="col-lg-12 col-md-12">
						...
						<p>
							...
							foreach ($indexes as $id => $value) {
								?>
								<li class="<?php if(array_key_exists($value, $answers)){print 'active ';}?>"><a href="<?php print $quiz_url.'/?qi='.$id;?>"><?php print $q+1 ?></a></li>
								<?php $q++; } ?>
								<?php if ($current_question+1==count($questions)): ?>
								<li class="min-w150 bg-orange"><a href="<?php print $quiz_url.'/?finish';?>">End Quiz</a>
								</li>
								<?php endif ?>
							</ul>
						</p>
	...
	<?php 
		...
		$js_date="$year"."-".sprintf('%02s',$month)."-".sprintf('%02s',$day)."T".get_time_from_minutes(get_minutes_from_time()+1).":".date('s').".000".date('P');
		}else{
		$js_date="$year"."-".sprintf('%02s',$month)."-".sprintf('%02s',$day)."T".get_time_from_minutes($attempt->end_time).":".date('s').".000".date('P');
		}
	 ?>
	<script type="text/javascript">
		...
		var x = setInterval(function() {
			...
			var d = count - now;	
			if(d <=0){
				<?php if($quiz->qbase_time<1){?>
			    window.location = "<?php print $quiz_url;?>/?next";
				<?php }else{ ?>
				location.reload();
				<?php } ?>
				clearInterval(x);
...
<script type="text/javascript">
	$('#goto').change(function() {
	    window.location = "<?php print $quiz_url;?>/?qi=" + $(this).val();
	});
</script>