...
<div class="wrapper _bg4586">
	...
	<div class="sa4d25">
		<div class="container-fluid">			
			<div class="row">
				<div class="col-lg-12">	
					<h2 class="st_title"><i class="uil uil-book-alt"></i>Paper Date Sheet</h2>
				...
				<div class="table-responsive mt-30">
					<table class="table ucp-table">
						...
						<tbody>
							...
							$filter['jd >=']=$todayjd;
							$papers=$this->paper_m->get_rows($filter,array('select'=>'name,date,jd,subject_id,class_id,section_id,start_time,allowed_time','orderby'=>'jd ASC, start_time ASC'));
							foreach($papers as $ppr){
								...
								if($this->subject_group_m->get_rows($filter,'',true)<1){
									...
									<td class="text-center">
										<?php if($ppr['jd']==$this->paper_m->todayjd){
											if(get_minutes_from_time()>($ppr['start_time']-$this->_TIME_MARGIN)){
													...
													}elseif($this->paper_attempt_m->get_rows(array('paper_id'=>$ppr['mid'],'student_id'=>$this->LOGIN_USER->mid),'',true)<1){ ?>
													<a class="btn btn-danger" href="<?php print $this->CONT_ROOT.'attempt/'.$ppr['mid'];?>">Start Paper</a>
												<?php }elseif($this->paper_attempt_m->get_rows(array('paper_id'=>$ppr['mid'],'student_id'=>$this->LOGIN_USER->mid,'is_ended'=>0),'',true)>0){ ?>
													<a class="btn btn-success" href="<?php print $this->CONT_ROOT.'attempt/'.$ppr['mid'];?>">Resume Paper</a>
												<?php }else{ ?>
													<span class="text-danger">Paper Attempted</span>
												<?php }
											}else{
												?>
