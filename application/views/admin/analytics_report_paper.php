				...
				<div class="col-md-10">
					<div class="section3125 rpt145">							
						<div class="row">						
							<div class="col-lg-7">
								<span class="_216b22">
									<a href="<?php print $this->LIB_CONT_ROOT.'detail/mdl/classes/'.$paper->class_id.'?tab=paper';?>" class="color-white">
							...
							foreach($students as $row){
							 ...
							 ?>
                                <tr>
									...
									<td class="cell-ta"><a href="<?php print $this->LIB_CONT_ROOT.'detail/mdl/students/'.$row['mid'];?>" title="View Profile"><?php print ucwords($row['name']); ?></a></td>
									<td class="text-center"><?php print $row['roll_number']; ?></td>
									<?php if(array_key_exists($row['mid'], $absentees)){
										if($this->paper_attempt_m->get_rows(array('student_id'=>$row['mid'],'paper_id'=>$paper->mid),'',true)>0){
											//student may have started the exam but not attempted any question
											?>
											<td class="text-center" colspan="3"><span class="text-warning">No Question Attempted</span></td>
											<td class="text-center">
												<a href="<?php print $this->CONT_ROOT.'report/removeattempt/'.$paper->mid.'/?type=paper&std='.$row['mid'];?>" onclick="return confirm('Are you sure to remove the student attempt. All answers submitted by student will be deleted regarding this attempt.');" title="Delete Student Attempt" class="text-danger">Delete Attempt</a>
											</td>
										<?php }else{
											//student have not yet started the exam
											?>
											<td class="text-center" colspan="4"><span class="text-danger">Not Attempted</span></td>
										<?php 
										} 
									}else{
										?>
										...
										<td class="text-center">
											<a href="<?php print $this->CONT_ROOT.'report/paperdetail/'.$paper->mid.'/?std='.$row['mid'];?>" title="View Report">View Paper</a>
											 | 
											<a href="<?php print $this->CONT_ROOT.'report/removeattempt/'.$paper->mid.'/?type=paper&std='.$row['mid'];?>" onclick="return confirm('Are you sure to remove the student attempt. All data submitted by student will be deleted regarding this attempt.');" title="Delete Student Attempt" class="text-danger">Delete Attempt</a>
										</td>
										<?php
									} ?>
								</tr>
