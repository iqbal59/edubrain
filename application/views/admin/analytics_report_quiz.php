							 ...
							 ?>
                                <tr>
									...
									<?php if(array_key_exists($row['mid'], $absentees)){
										if($this->quiz_attempt_m->get_rows(array('student_id'=>$row['mid'],'quiz_id'=>$quiz->mid),'',true)>0){
											...
											<td class="text-center">
												<a href="<?php print $this->CONT_ROOT.'report/removeattempt/'.$quiz->mid.'/?type=quiz&std='.$row['mid'];?>" onclick="return confirm('Are you sure to remove the student attempt. All answers submitted by student will be deleted regarding this attempt.');" title="Delete Student Attempt" class="text-danger">Delete Attempt</a>
											</td>
										<?php }else{
