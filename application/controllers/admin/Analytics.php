...
*/  
    ...
    public function report($module='',$rid=''){
        ...
        $like=array();
        $form=$this->input->safe_get();
        $redir=$this->CONT_ROOT.'report/'.strtolower($module).'/'.$rid;
        switch (strtolower($module)) {
            ...
            /////////load row
            case 'testdetail':{
                $this->data['main_content']='analytics_report_test_detail';  
                $this->data['menu']='analytics';            
                $this->data['sub_menu']='analytics';
                //////////////////////////////////////
                $this->data['groups']=$this->group_m->get_values_array('','name',array());
                ...
                $this->data['class']=$this->class_m->get_by_primary($row->class_id);
                $this->data['subject']=$this->subject_m->get_by_primary($row->subject_id);
                $this->data['student']=$this->student_m->get_by_primary($row->student_id);
                $this->data['sections']=$this->section_m->get_values_array('','name',array());
                $this->data['answers']=$this->test_answer_m->get_values_array('question_id','answer',array('test_id'=>$row->mid,'student_id'=>$row->student_id));
                $this->data['attempts']=$this->test_attempt_m->get_by(array('test_id'=>$row->mid,'student_id'=>$row->student_id),true);
                $this->data['questions']=$this->test_question_m->get_rows(array('test_id'=>$row->mid),array('select'=>'question,option1,option2,option3,option4,answer,marks,'));
            }
            break;  
            /////////load row
            case 'removeattempt':{
                ...
                switch(strtolower($form['type'])){
                    case 'quiz' : {
                        $student=$this->student_m->get_by_primary($form['std']);
                        $row=$this->quiz_m->get_by_primary($rid);
                        $this->quiz_attempt_m->delete(NULL,array('quiz_id'=>$row->mid,'student_id'=>$student->mid));
                        $this->quiz_answer_m->delete(NULL,array('quiz_id'=>$row->mid,'student_id'=>$student->mid,'subject_id'=>$row->subject_id));
                        $this->session->set_flashdata('success', $student->name.' attempt has been removed regarding this exam.');
                        redirect($this->CONT_ROOT.'report/quiz/'.$rid, 'refresh');  
                    ...
                    case 'paper' : {
                        $student=$this->student_m->get_by_primary($form['std']);
                        $row=$this->paper_m->get_by_primary($rid);
                        $this->paper_attempt_m->delete(NULL,array('paper_id'=>$row->mid,'student_id'=>$student->mid));
                        $this->paper_answer_m->delete(NULL,array('paper_id'=>$row->mid,'student_id'=>$student->mid,'subject_id'=>$row->subject_id));
                        $this->paper_doc_m->delete(NULL,array('paper_id'=>$row->mid,'student_id'=>$student->mid,'subject_id'=>$row->subject_id));
                        $this->session->set_flashdata('success', $student->name.' attempt has been removed regarding this paper.');
                        redirect($this->CONT_ROOT.'report/paper/'.$rid, 'refresh');  
