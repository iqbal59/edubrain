...
class Generate extends Staff_Controller{
    ...
    public function mdl($module='',$rid=''){
        ...
        switch (strtolower($module)) {
            ...
            case 'quiz':{
                ...
                //////////////////////////////////////////////
                if(empty($rid)){$rid=$form['sid'];}
                $this->data['subject']=$this->subject_m->get_by_primary($rid);
                $this->data['class']=$this->class_m->get_by_primary($this->data['subject']->class_id);
                $this->data['subjects']=$this->subject_m->get_values_array('','name',array('class_id'=>$this->data['subject']->class_id));
            ...
            case 'quizbank':{
                ...
                if($this->user_m->todayjd > $this->data['quiz']->jd){
                    ...
                    redirect($this->LIB_CONT_ROOT.'index/quiz/?sid='.$this->data['quiz']->subject_id);                    
                }elseif(get_minutes_from_time() >= ($this->data['quiz']->start_time-5) && $this->user_m->todayjd == $this->data['quiz']->jd){
                    $this->session->set_flashdata('error', 'Unable to edit because paper start time has been passed.');
                    redirect($this->LIB_CONT_ROOT.'index/quiz/?sid='.$this->data['quiz']->subject_id);                    
                }                
                //////////////////////////////////////////////////////////////////////////////////
            ...
            case 'paperbank':{
                ...
                if($this->user_m->todayjd > $this->data['paper']->jd){
                    $this->session->set_flashdata('error', 'Unable to edit because paper start time has been passed.');
                    redirect($this->LIB_CONT_ROOT.'index/paper/?sid='.$this->data['paper']->subject_id);                    
                }elseif(get_minutes_from_time() >= ($this->data['paper']->start_time-5) && $this->user_m->todayjd == $this->data['paper']->jd){
                    $this->session->set_flashdata('error', 'Unable to edit because paper start time has been passed.');
                    redirect($this->LIB_CONT_ROOT.'index/paper/?sid='.$this->data['paper']->subject_id);                    
                }
