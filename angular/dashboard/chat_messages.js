...
app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys){
    ...
    $scope.loadRows=function (){
		...
		$http.get($scope.ajax_url+'filterMessages',config.postConfig).then(function(response){
			$scope.enableButtons();
	...
	$scope.sendMessage=function (){
		...
		$http.post($scope.ajax_url+'addMessage',data,config.postConfig).then(function(response){
	...
	///////autoload functions////////////////////
	$timeout(function(){$scope.loadRows();},200);//SET INTERVAL TO AUTO UPDATE THE ROWS
