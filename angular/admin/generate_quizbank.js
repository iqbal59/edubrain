...
app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys){
	...
	$scope.record={};
	///////////////////////////////
	$scope.filter={chapter:'',limit:'',chapter:''};
    ...
    $scope.loadRow=function (){
		...
		$http.get($scope.ajax_url+'load/quiz',config.postConfig).then(function(response){
			$scope.enableButtons();
    ...
    $scope.loadRows=function (){
		...
		config.postConfig.params=data;
		$scope.disableButtons();
		$http.get($scope.ajax_url+'filter/quizbank',config.postConfig).then(function(response){
			$scope.enableButtons();
    ...
    $scope.loadQuestionBank=function (){
		var data={'rid':$scope.rid,'chapter':$scope.filter.chapter,'limit':$scope.filter.limit};
		config.postConfig.params=data;
		$scope.disableButtons();
	...
	$scope.saveRow=function (){	
		...
		$http.post($scope.ajax_url+'update/manualquizbank',data,config.postConfig).then(function(response){
	...
	$scope.saveQbRow=function (qid){	
		...
		data+='&qid='+qid;
		data+='&rid='+encodeURIComponent($scope.rid);
		$http.post($scope.ajax_url+'update/qbquizbank',data,config.postConfig).then(function(response){
			var msg=response.data.message;
			if(response.data.error === true){
				alert(msg);
				$log.error(response);
	...
	$scope.delRow=function (id){	
		...
		var data='';
		data+='&rid='+id;
		$http.post($scope.ajax_url+'delete/quizbank',data,config.postConfig).then(function(response){
			var msg=response.data.message;
			if(response.data.error === true){
				alert(msg);
				$log.error(response);
	...
	///////autoload functions////////////////////
	$timeout(function(){$scope.loadRow();},1000); 
